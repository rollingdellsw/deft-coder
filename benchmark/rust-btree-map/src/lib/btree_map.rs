    ...
    K: Ord,
{
        ...
        let child = RefCell::borrow(node_ref).children[child_idx]
            ...
            .unwrap();
          {
            ...
            if !child_node.is_leaf() {
                let left_child = {
                    let left = RefCell::borrow(&left_sibling);
                    left.children[left_len].clone()
                };
                for i in (1..=child_len).rev() {
    ...
    fn rotate_left(&mut self, node_ref: &Rc<RefCell<Node<K, V>>>, child_idx: usize) {
            ...
            .unwrap();
        {
            ...
            if !child_node.is_leaf() {
                let right_child = {
                    let right = RefCell::borrow(&right_sibling);
            ...
            if !left_mut.is_leaf() {
                unsafe {
                    ptr::copy_nonoverlapping(
                        ...
                        left_mut.children.as_mut_ptr().add(left_len + 1),
                        child_len + 1,
                    );
                }
            }
