...
You can implement custom retry logic using the `ctx.memory` object:

```javascript
output: async (ctx) => {
  // Track retries per tool
  const retryKey = `${ctx.tool.name}_output_retries`;
  const retries = ctx.memory.get(retryKey) ?? 0;

  // Give up after N retries
  if (retries >= 5) {
    console.log(
      `[Guardrail] Max retries reached for ${ctx.tool.name}, allowing`,
    );
  ...
  if (ctx.tool.name === "patch" && !ctx.result.isError) {
    ...
    if (exitCode !== 0) {
      ctx.memory.set(retryKey, retries + 1);
      return { override: true, isError: true, result: "Tests failed" };
    }
  }

  // Reset counter on success
  ctx.memory.set(retryKey, 0);
  return { override: false };
};
```
...
1. `.deft/guardrails.js` (project-local)
