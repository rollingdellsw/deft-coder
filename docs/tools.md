...
```javascript
// Simple evaluation
sandbox_ts({ cmd: "node -e 'console.log(1+1)'" });

// Multi-step commands
sandbox_ts({
  cmd: "npm init -y && npm install lodash && node -e 'console.log(require(\"lodash\").chunk([1,2,3,4], 2))'",
});

// TypeScript compilation check
sandbox_ts({
  cmd: "cat > /tmp/test.ts << 'EOF'\nconst x: number = 'hello';\nEOF\nnpx tsc --noEmit /tmp/test.ts",
});
```
...
| `scope`     | string | `"all"`, `"src"`, `"tests"`, or directory path |
...
| `timeoutMs`            | number   | Timeout in ms (default 300000)                 |
| `allowed_tools`        | string[] | Tools the subtask agent can use                |
...
// List directory
list_files({ path: "src" });
list_files({ path: ".", includeHidden: true });
...
| **Executor** (mgrep) | Read-only MCP tools (no mgrep to prevent recursion) |
...
| Task           | Tool              | Example                                               |
| -------------- | ----------------- | ----------------------------------------------------- |
| Run TypeScript | `sandbox_ts`      | `sandbox_ts({ cmd: "npx ts-node script.ts" })`        |
| Run Node       | `sandbox_ts`      | `sandbox_ts({ cmd: "node -e 'console.log(1)'" })`     |
