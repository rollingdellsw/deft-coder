...
export interface MCPServerConfig {
  ...
  tools: ToolHandler[];
  validatePath?: (requestedPath: string, workingDir: string) => string;
}
...
export function validatePath(
  ...
  try {
    // IMPORTANT: Resolve working directory to absolute path first
    // This handles cases where workingDirectory is "." or relative
...
export function createServer(config: MCPServerConfig) {
  ...
  async function handleRequest(request: MCPRequest) {
    ...
    if (params !== undefined && Object.keys(params).length > 0) {
      printDebug(`   Params: ${JSON.stringify(params, null, 2)}`);
    }
  ...
  return {
    start() {
      ...
      rl.on("line", (line) => {
        ...
        printDebug(
          `[MCP Server] Received line: ${line.slice(0, 100)}${line.length > 100 ? "..." : ""}`,
        );
